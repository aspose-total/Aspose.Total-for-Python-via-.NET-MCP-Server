#!/usr/bin/env python3
"""Aspose.CAD standalone MCP server.

Serves only the CAD plugin — no other Aspose products required.
Cross-platform: Windows x32/x64, Linux, macOS.
Requires Python >= 3.9.

Usage:
    python servers/cad/server.py                              # stdio
...
load_dotenv(_SERVER_DIR / ".env")   # servers/cad/.env — product-specific overrides
...
from mcp_server.plugins.cad_plugin import CadPlugin  # type: ignore[import-untyped]

_MCP_INSTRUCTIONS = (
    "This server converts and processes CAD/BIM drawings using Aspose.CAD. "
    "Supports AutoCAD DWG/DXF, BIM IFC, MicroStation DGN, and 20+ other CAD formats. "
    ...
    "Pass only the bare filename (e.g. 'drawing.dwg') to file parameters — never a full path. "
...
def main() -> None:
    """Entry point for the Aspose CAD standalone MCP server."""
    parser = argparse.ArgumentParser(description="Aspose CAD standalone MCP server")
    parser.add_argument(
        "--transport",
    ...
    mcp = FastMCP("Aspose CAD MCP Server", instructions=_MCP_INSTRUCTIONS)
    ...
    logger.info("Aspose CAD MCP Server starting (transport=%s)", args.transport)

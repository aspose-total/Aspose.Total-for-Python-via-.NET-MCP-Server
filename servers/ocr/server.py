#!/usr/bin/env python3
"""Aspose OCR MCP Server — standalone single-plugin process."""
...
from mcp_server.plugins.ocr_plugin import OcrPlugin

SERVER_VERSION = "0.1.0"
_MCP_INSTRUCTIONS = (
    "This server extracts text from images and scanned documents using Aspose.OCR. "
    "All tools work with files already present in the server's ASPOSE_FILES_PATH folder. "
    "Pass only the bare filename (e.g. 'scan.jpg') to file parameters — never a full path. "
    "Output files are written to the same ASPOSE_FILES_PATH directory."
)


def main() -> None:
    mcp = FastMCP("Aspose OCR MCP Server", instructions=_MCP_INSTRUCTIONS)

    plugin = OcrPlugin()
    plugin.register_tools(mcp)
    logger.info("Registered %d tools from ocr plugin.", len(plugin.get_tool_manifest()))

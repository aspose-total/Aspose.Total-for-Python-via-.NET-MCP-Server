#!/usr/bin/env python3
"""Aspose PDF MCP Server — standalone single-plugin process."""
...
from mcp_server.plugins.pdf_plugin import PdfPlugin

SERVER_VERSION = "0.1.0"
_MCP_INSTRUCTIONS = (
    "This server processes PDF documents using Aspose.PDF. "
    "All tools work with files already present in the server's ASPOSE_FILES_PATH folder. "
    "Pass only the bare filename (e.g. 'report.pdf') to file parameters — never a full path. "
    "Output files are written to the same ASPOSE_FILES_PATH directory."
)


def main() -> None:
    mcp = FastMCP("Aspose PDF MCP Server", instructions=_MCP_INSTRUCTIONS)

    plugin = PdfPlugin()
    plugin.register_tools(mcp)
    logger.info("Registered %d tools from pdf plugin.", len(plugin.get_tool_manifest()))

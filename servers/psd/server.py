#!/usr/bin/env python3
"""Aspose.PSD standalone MCP server.

Serves only the PSD plugin — no other Aspose products required.
Cross-platform: Windows x32/x64, Linux x64, macOS x64/Arm64.

Usage:
    python servers/psd/server.py                              # stdio
...
from mcp_server.plugins.psd_plugin import PsdPlugin  # type: ignore[import-untyped]

_MCP_INSTRUCTIONS = (
    "This server processes Adobe Photoshop PSD/PSB files using Aspose.PSD. "
    "All tools work with files already present in the server's ASPOSE_INPUT_DIR folder. "
    "Pass only the bare filename (e.g. 'design.psd') to file parameters — never a full path. "
...
def main() -> None:
    """Entry point for the Aspose PSD standalone MCP server."""
    parser = argparse.ArgumentParser(description="Aspose PSD standalone MCP server")
    parser.add_argument(
        "--transport",
    ...
    mcp = FastMCP("Aspose PSD MCP Server", instructions=_MCP_INSTRUCTIONS)
    ...
    logger.info("Aspose PSD MCP Server starting (transport=%s)", args.transport)

#!/usr/bin/env python3
"""Aspose.SVG standalone MCP server.

Serves only the SVG plugin — no other Aspose products required.
Import path assumes this file is run from the repo root or the
servers/svg/ directory, both of which have mcp_server/ accessible.

PLATFORM NOTE: aspose-svg-net is Windows only (x86/x64).
This server will start on any platform but the svg plugin will report
health_check() = False on Linux/macOS. All tools will raise
RuntimeError on non-Windows platforms.

Usage:
    python servers/svg/server.py                              # stdio
    python servers/svg/server.py --transport streamable-http --port 8003
    python servers/svg/server.py --transport sse --port 8003
"""
...
from mcp_server.plugins.svg_plugin import SvgPlugin  # type: ignore[import-untyped]

_MCP_INSTRUCTIONS = (
    "This server creates, edits, converts, and vectorizes SVG documents using Aspose.SVG. "
    "All tools work with files already present in the server's ASPOSE_INPUT_DIR folder. "
    "Pass only the bare filename (e.g. 'logo.svg') to file parameters — never a full path. "
...
def main() -> None:
    """Entry point for the Aspose SVG standalone MCP server."""
    import argparse

    parser = argparse.ArgumentParser(description="Aspose SVG standalone MCP server")
    parser.add_argument(
        "--transport",
    ...
    mcp = FastMCP("Aspose SVG MCP Server", instructions=_MCP_INSTRUCTIONS)
    ...
    # register_tools calls apply_license_for("aspose.svg") internally
    plugin = SvgPlugin()
    plugin.register_tools(mcp)

    logger.info("Aspose SVG MCP Server starting (transport=%s)", args.transport)

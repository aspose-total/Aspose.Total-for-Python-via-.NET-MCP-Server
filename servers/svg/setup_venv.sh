...
echo "⚠️  WARNING: aspose-svg-net is Windows only (x86/x64)."
...
pip install -r "${SCRIPT_DIR}/requirements.txt" --quiet || {
    ...
    echo "   aspose-svg-net only has Windows wheels on PyPI."
...
echo ""
echo "✅ Setup complete!"
echo "   Note: svg_plugin.py tools only work on Windows at runtime."
echo ""
echo "To start the server: python servers/svg/server.py"

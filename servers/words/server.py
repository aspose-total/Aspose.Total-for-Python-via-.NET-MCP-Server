#!/usr/bin/env python3
"""Aspose Words MCP Server — standalone single-plugin process."""
...
from mcp_server.plugins.words_plugin import WordsPlugin

SERVER_VERSION = "0.1.0"
_MCP_INSTRUCTIONS = (
    "This server processes Word documents using Aspose.Words. "
    "All tools work with files already present in the server's ASPOSE_FILES_PATH folder. "
    "Pass only the bare filename (e.g. 'report.docx') to file parameters — never a full path. "
    "Output files are written to the same ASPOSE_FILES_PATH directory."
)


def main() -> None:
    mcp = FastMCP("Aspose Words MCP Server", instructions=_MCP_INSTRUCTIONS)

    plugin = WordsPlugin()
    plugin.register_tools(mcp)
    logger.info("Registered %d tools from words plugin.", len(plugin.get_tool_manifest()))
